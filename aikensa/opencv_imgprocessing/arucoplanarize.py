        ...
        if corners and ids is not None:
            ...
            for i, corner in zip(ids, corners):
                ...
                if marker_id == 0:
                    ...
                    bottom_left_corner = corner[0][2]
                elif marker_id == 3:
                    # Top right corner of marker 3
                    bottom_right_corner = corner[0][3]
    ...
    if corners and ids is not None:
        ...
        for i, corner in zip(ids, corners):
            ...
            if marker_id == 0:
                # Top left corner of marker 0
                # top_left_corner = corner[0][0]
                #change to bottom left corner of marker 0
                top_left_corner = corner[0][3]
            elif marker_id == 1:
                # Bottom right corner of marker 1
                top_right_corner = corner[0][2]
            elif marker_id == 2:
                # Top right left corner of marker 2
                bottom_left_corner = corner[0][0]
            elif marker_id == 3:
                # Top left corner of marker 3
...
def planarize_image_4567(image, target_width, target_height, top_offset, bottom_offset):
    transform = None
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    desired_plane = np.array([[0, 0], [target_width, 0], [0, target_height], [target_width, target_height]], dtype='float32')
    corners, ids, rejected = detector.detectMarkers(gray)
...
def planarize_image_wide(image, target_width, target_height, top_offset, bottom_offset, side_offset=0):
    transform = None
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    desired_plane = np.array([[0, 0], [target_width, 0], [0, target_height], [target_width, target_height]], dtype='float32')
    corners, ids, rejected = detector.detectMarkers(gray)
    ...
    if corners and ids is not None:
        ...
        and bottom_left_corner is not None and bottom_right_corner is not None:
            ...
            desired_plane = np.array([[0 + side_offset, top_offset], [target_width - side_offset, top_offset], [0 + side_offset, target_height-bottom_offset], [target_width - side_offset, target_height-bottom_offset]], dtype='float32')
...
def planarize_image_narrow(image, target_width, target_height, top_offset, bottom_offset, side_offset=0):
    transform = None
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    desired_plane = np.array([[0, 0], [target_width, 0], [0, target_height], [target_width, target_height]], dtype='float32')
    corners, ids, rejected = detector.detectMarkers(gray)
    ...
    if corners and ids is not None:
        ...
        for i, corner in zip(ids, corners):
            ...
            if marker_id == 4:
                # Top left corner of marker 0
                top_left_corner = corner[0][2]
            elif marker_id == 5:
                # Top right corner of marker 1
                top_right_corner = corner[0][3]
            elif marker_id == 6:
                # Bottom left corner of marker 2
                bottom_left_corner = corner[0][1]
            elif marker_id == 7:
                # Bottom right corner of marker 3
                bottom_right_corner = corner[0][0]
