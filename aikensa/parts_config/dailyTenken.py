...
def dailyTenken(image, sahi_predictionList):
    ...
    #Measured length by calculating the distance between the most left of the left detection and the most right of the right detection
    length = 0
    if len(detectedposX) > 1:
        leftmost = detectedposX[0] - detectedWidth[0] / 2
        rightmost = detectedposX[-1] + detectedWidth[-1] / 2

        length = rightmost - leftmost
        length = length * pixelMultiplier
        measuredPitch.append(length)
