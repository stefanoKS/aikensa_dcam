...
from aikensa.parts_config.sound import play_do_sound, play_re_sound, play_mi_sound
...
class CameraThread(QThread):
    ...
    def run(self):
        ...
        while self.running is True:
            ...
            if self.cam_config.widget == 3:
                # self.partCheck(frame1, frame2, ret1, ret2, self.cam_config.widget)
                ...
                if self.cam_config.HDRes == False:
                    ...
                    clipFrame2 = self.frameCrop(frame1, x=int(1900/self.scale_factor), y=int(0/self.scale_factor), w=int(600/self.scale_factor), h=int(600/self.scale_factor), wout = 128, hout = 128)
                    clipFrame3 = self.frameCrop(frame2, x=int(600/self.scale_factor), y=int(0/self.scale_factor), w=int(600/self.scale_factor), h=int(600/self.scale_factor), wout = 128, hout = 128)

                if self.cam_config.HDRes == True:
                    cv2.imwrite("frame1ref.jpg", frame1)
                    cv2.imwrite("frame2ref.jpg", frame2)
                    clipFrame1 = self.frameCrop(frame1, x=590, y=0, w=600, h=600, wout = 128, hout = 128)
                    clipFrame2 = self.frameCrop(frame1, x=1900, y=0, w=600, h=600, wout = 128, hout = 128)
                    clipFrame3 = self.frameCrop(frame2, x=600, y=0, w=600, h=600, wout = 128, hout = 128)
                    

                if self.handClassificationModel is not None:
                ...
                if self.cam_config.triggerKensa == True or self.oneLoop == True:
                    ...
                    if self.oneLoop == True:
                        self.clip_detection = get_sliced_prediction(combinedImage, self.ctrplr_clipDetectionModel, slice_height=512, slice_width=512, overlap_height_ratio=0.2, overlap_width_ratio=0.2)
                        print("Clip Detection: ", self.clip_detection.object_prediction_list)
                        imgcheck, pitch_results, detected_pitch, delta_pitch, hanire = ctrplrCheck(combinedImage, self.clip_detection, detections_hanire, partid="LH")
                        self.clip_detection = None
                        self.oneLoop = False
                ...
                self.handFrame1.emit(self.result_handframe1)
                self.handFrame2.emit(self.result_handframe2)
                self.handFrame3.emit(self.result_handframe3)
    ...
    def initialize_model(self):
        ...
        print("ClipDetectionModel initialized.")